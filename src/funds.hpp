...
private:
  constexpr Funds(uint64_t val) : IsUint64(val) {};
...
public:
  ...
  void add_throw(Funds add) { *this = sum_throw(*this, add); }
  void add_assert(Funds add) { *this = sum_assert(*this, add); }
  ...
  template <typename... T> static Funds sum_throw(Funds a, T &&...t) {
  ...
  template <typename... T> static Funds sum_assert(Funds a, T &&...t) {
    auto s{sum(a, std::forward<T>(t)...)};
    assert(s.has_value());
    return *s;
  }

  Funds &subtract_assert(Funds f) {
